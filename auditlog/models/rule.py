...
class AuditlogRule(models.Model):
    ...
    def create_logs(
        ...
        for res_id in res_ids:
            ...
            auditlog_rule_link = '<a href="%s%s">%s</a>' % (
                ...
                "/web#id=%d&view_type=form&model=%s"
                % (auditlog_rule.id, auditlog_rule._name),  # Record URL
            ...
            modified_object_link = '<a href="%s%s">%s</a>' % (
                ...
                .get_param("web.base.url"),  # Base URL
                "/web#id=%d&view_type=form&model=%s"
                % (res_id, res_model),  # Modified object URL
            ...
            ticket_text += (
                "This department was notified according to Audit Rule: %s"
                % auditlog_rule_link
            )
    ...
    def _compute_records_monitored_display(self):
        for record in self:
            record.records_monitored_display = (
                "Number of records that will be monitored: %s"
                % record.records_monitored_amount
            )
